...
def get_token() -> str:
    """Get a VW Group IDP access token using the verified client-credentials flow."""
...
def init_llmaas():
    """Create the verified OpenAI-compatible VW Group LLMaaS client."""
    ...
    return OpenAI(
        ...
        default_headers={
            "X-LLM-API-CLIENT-ID": f"Bearer {api_client_id}"
        },
    )
...
def _llm_complete(
    ...
    """Call VW Group LLMaaS using the verified Chat Completions pattern."""
    if not enabled():
        raise RuntimeError(
            "VW Group LLMaaS is not configured. Configure the VW Group secrets in Streamlit."
        )
...
def get_embedding(input_text: str) -> dict:
    """Create an embedding with the verified embedding model."""
    if not enabled():
        raise RuntimeError(
            "VW Group LLMaaS is not configured. Configure the VW Group secrets in Streamlit."
        )
...
def generate_root_cause(case: dict, model: str | None = None) -> str:
    """Generate a concise, evidence-grounded RCA brief using VW Group LLMaaS."""
    if not enabled():
        raise RuntimeError(
            "VW Group LLMaaS is not configured. Configure the VW Group secrets in Streamlit."
        )
...
Use ONLY the supplied case data and linked workbook evidence.

CASE DATA:
{json.dumps(case, indent=2, default=str)}

Create a concise operator-facing decision brief.

Rules:
1. Preserve exact IDs, quantities, statuses, and dates from the supplied evidence.
2. Do not invent missing information.
3. Clearly distinguish physical on-hand, blocked stock, usable available stock,
   demand, shortage, inbound, delivery status, warehouse capacity, and vendor/PO status.
4. Do not simply copy the supplied root_cause field. Rewrite it into a clear causal chain.
5. Identify one primary root cause and separate contributing factors.
6. Recommendations are proposals only.
7. Keep the response short and easy to scan.

Return exactly:

### Primary Root Cause
1–3 sentences explaining the main causal chain and the most important quantities.

### Contributing Factors
3–5 bullets with concrete supporting facts.

### Operational Impact
1 short paragraph describing the operational effect supported by the evidence.

### Recommended Action
1–2 concise proposed actions.
